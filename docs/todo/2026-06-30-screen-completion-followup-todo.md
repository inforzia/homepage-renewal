...
- `SiteHero` 범용 컴포넌트는 생성되었고, 현재 `/`, `/vision-ai`, `/solutions/[id]` hero에서 사용 중이다.
- 단순 hero 래퍼 컴포넌트를 추가로 분리하지 않고, 각 페이지에서 직접 `SiteHero`를 사용하는 방향으로 정리되었다.
- `company/about` 페이지가 추가되었고, `Company` 메뉴의 `About Inforzia` 진입점이 해당 경로로 연결되도록 정리되었다.
- 임시 산출물은 `/.temp/` 아래로 정리하는 규칙이 `AGENTS.md`에 반영되었다.

## 다음 재개 포인트

1. `문의하기` 화면 수정 범위를 먼저 정리하고 `company/about`과의 역할 분담을 확정한다.
2. `use-cases` 하단 상담 영역을 `company/contact-us`와 어떻게 연결할지 정리한다.
3. 공통 CTA, `더 보기`, `소개서 다운로드`, `개인정보 처리방침` 등 링크 목적지를 정리한다.
4. `Flomon Vision` 페이지의 실제 미완료 인터랙션이 필요한지, 정적 표현으로 둘지 결정한다.
...
- 관련 TODO:
    ...
    - [2. 공통 hero 구조 정리 및 주요 페이지 적용](#2-공통-hero-구조-정리-및-주요-페이지-적용)
- 참고 자료:
    - `src/app/page.tsx`
    - `src/app/vision-ai/page.tsx`
...
- 관련 TODO:
    - [6. 공통 링크와 placeholder 상호작용 정리](#6-공통-링크와-placeholder-상호작용-정리)
...
- 관련 TODO:
    - [7. Flomon Vision 페이지 미완료 항목 정리](#7-flomon-vision-페이지-미완료-항목-정리-결정-필요)
- 참고 자료:
    - [Flomon Vision 페이지 구현 TODO](./flomon-vision-page-todo.md)
...
- 관련 TODO:
    - [3. Company 진입 화면 개발](#3-company-진입-화면-개발)
    - [4. 문의하기 화면 수정](#4-문의하기-화면-수정)
...
- 공통 hero 기반인 `SiteHero`가 솔루션 및 주요 일반 페이지에 안정적으로 적용되고, 불필요한 페이지별 래퍼 컴포넌트가 제거된다.
...
- [x] [2. 공통 hero 구조 정리 및 주요 페이지 적용](#2-공통-hero-구조-정리-및-주요-페이지-적용)
- [x] [3. Company 진입 화면 개발](#3-company-진입-화면-개발)
- [ ] [4. 문의하기 화면 수정](#4-문의하기-화면-수정)
- [ ] [5. use cases 상세 화면 하단 상담 화면 개발](#5-use-cases-상세-화면-하단-상담-화면-개발)
- [ ] [6. 공통 링크와 placeholder 상호작용 정리](#6-공통-링크와-placeholder-상호작용-정리)
- [ ] [7. Flomon Vision 페이지 미완료 항목 정리 (결정 필요)](#7-flomon-vision-페이지-미완료-항목-정리-결정-필요)
- [ ] [8. 상단 메뉴 하위 메뉴들의 interaction 추가](#8-상단-메뉴-하위-메뉴들의-interaction-추가)
...
### 2. 공통 hero 구조 정리 및 주요 페이지 적용

- 상위 TODO: [TODO 목록으로 이동](#todo-목록)
- 목적:
    - 공통 hero 기반을 정리하고, 필요한 페이지에 과도한 래퍼 분리 없이 안정적으로 적용한다.
- 작업 내용:
    - [x] 이미지/영상 배경, eyebrow, title, description, color를 받을 수 있는 공통 `SiteHero`를 정리한다.
    - [x] 솔루션 페이지 hero를 `SiteHero` 기반으로 통합한다.
    - [x] `/`와 `/vision-ai` hero를 `SiteHero` 기반 구현으로 정리한다.
    - [x] 단순하고 재사용되지 않는 hero 전용 래퍼 컴포넌트는 추가로 두지 않는다.
    - [x] 적용 이후에도 기존 시맨틱 구조, 링크 동작, 정적 호스팅 전제를 유지하는지 확인한다.
- 진행 메모:
    - `src/components/site/SiteHero.tsx` 범용 hero 컴포넌트를 먼저 추출하고 솔루션 hero 구조를 정리했다.
    - 이후 `/`와 `/vision-ai`도 페이지 내부에서 직접 `SiteHero`를 사용하도록 정리했다.
    - `HomeHero`, `VisionAiHero`처럼 단순 래퍼 역할의 hero 컴포넌트는 유지하지 않는 방향으로 정리되었다.
- 참고 자료:
    - `src/app/page.tsx`
    - `src/app/vision-ai/page.tsx`
    - `src/components/site/SiteHero.tsx`
...
- 작업 내용:
- [x] 현재 `Company` 메뉴가 어떤 경로와 화면으로 연결되는지 점검한다.
- [x] `Company` 전용 진입 화면이 필요한지, `contact-us` 재사용이 맞는지 정리한다.
- [x] 독립 화면이 필요하다면 포함할 섹션(회사 소개, 미션, 연락처, CTA)을 정리한다.
- [x] 헤더 dropdown과 `Company` 대표 경로 설계에 필요한 후속 작업을 정리한다.
- [x] 실제 화면 구현이 필요한 경우 별도 구현 task를 추가한다.
- 진행 메모:
    - `https://seed-bronze-70048020.figma.site/about`를 기준으로 `src/app/company/about/page.tsx` 화면을 구현했다.
    - `Company` 메뉴의 `About Inforzia` 링크는 `/company/about`으로 변경했다.
    - `src/app/sitemap.ts`에도 `company/about` 경로를 추가했다.
    - 중간 비즈니스 카드 이미지는 참조 페이지 자산 노출 한계로 인해 현재 프로젝트 로컬 자산으로 대체했다.
- 참고 자료:
...
### 7. Flomon Vision 페이지 미완료 항목 정리 (결정 필요)
...
- 변경된 주요 내용:
    ...
    - `/`와 `/vision-ai` hero도 페이지 내부에서 직접 `SiteHero`를 사용하는 구조로 정리했다.
    - 솔루션 화면은 `/solutions/[id]` 동적 라우트로 통합되었고, 데이터는 `src/datas/solutions.tsx`로 이동했다.
    - `company/about` 페이지를 추가하고, `Company` 메뉴의 대표 진입점을 해당 화면으로 연결했다.
    ...
    - `HomeHero`, `VisionAiHero`처럼 단순 래퍼 역할의 hero 컴포넌트는 두지 않는 방향으로 정리했다.
    - 임시 파일 저장 위치를 `/.temp/`로 통일하는 규칙을 `AGENTS.md`에 반영했다.
- 남은 제약 또는 후속 작업:
    - 다음 우선 작업은 `문의하기` 화면 정리이며, 이후 하단 상담 섹션 연결 범위를 함께 정리해야 한다.
    - 공통 CTA와 placeholder 링크 정리는 아직 남아 있어 실제 목적지 확정이 필요하다.
    - `Flomon Vision` 페이지의 상호작용 필요 여부는 여전히 사용자 결정이 필요한 상태다.
